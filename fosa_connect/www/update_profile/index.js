...
// Function to hide and show respective divs on navbar click
let navItems = Array.from(document.getElementById('home-navbar').children);
navItems.forEach(navItem => {
  navItem.addEventListener('click', function () {
    showDiv(navItem.className, navItems);
  });
});
function showDiv(divClass, navItems) {
  // Hide all divs
  navItems.forEach(function (div) {
    document.getElementById(div.className + '-fields').style.display = 'none';
    document.querySelector(('.' + div.className + '-line')).classList.remove('blue-line');
  });
  document.getElementById((divClass + '-fields')).style.display = 'flex';
  document.querySelector(('.' + divClass + '-line')).classList.add('blue-line');
}
...
$(document).ready(function () {
  ...
  document.getElementById('uploadImg').addEventListener('change', function () {
    setTimeout(function () {
      ...
      frappe.call({
        method: 'fosa_connect.www.update_profile.index.upload_image',
...
const validateForm = (className) => {
  let elements = document.getElementsByClassName(className);
  for (const element of elements) {
    if (element.id) {
      if (!element.value) {
        frappe.throw("Please fill " + element.name);
      }
    }
  };
};
...
const updateMember = () => {
  try {
    validateForm('input');
    console.log('Validation passed. Continuing with the update process.');

    const full_name = $("#full_name").val().trim();
    const nameParts = full_name.split(' ');
    const first_name = frappe.utils.xss_sanitise(nameParts[0] || "").trim();
    const middle_name = frappe.utils.xss_sanitise(nameParts.slice(1, -1).join(' ') || "").trim();
    const last_name = frappe.utils.xss_sanitise(nameParts.slice(-1).join(' ') || "").trim();
    ...
    const career_objective_or_summary = frappe.utils.xss_sanitise($("#bio").val() || "").trim();
    ...
    const country = frappe.utils.xss_sanitise($('#country').val() || "").trim();
    const career_history = get_career_history_data();
    ...
    frappe.call({
      method: "fosa_connect.www.update_profile.index.update_member",
      args: {
...
function add_career_history_row() {
  var table = document.getElementById("career-history").getElementsByTagName('tbody')[0];
  ...
  orgNameCell.innerHTML = '<input class="table-input" type="text" name="organization_name[]" value="" />';
  industryCell.innerHTML = '<input class="table-input" type="text" name="industry[]" value="" />';
  roleCell.innerHTML = '<input class="table-input" type="text" name="role[]" value="" />';
  orgdecCell.innerHTML = '<input class="table-input" type="text" name="organization_description[]" value="" />';
  fromCell.innerHTML = '<input class="table-input" type="date" name="from_date[]" value="" />';
  toCell.innerHTML = '<input class="table-input" type="date" name="to_date[]" value="" />';
  currentCell.innerHTML = '<input class="table-input" type="checkbox" name="curr_date[]" onchange="toggleToDate(this)"/>';
...
function add_education_row() {
  ...
  degrees_earnedCell.innerHTML = '<input class="table-input" type="text" name="degrees_earned[]" value="" />';
  majorfield_of_studyCell.innerHTML = '<input class="table-input" type="text" name="majorfield_of_study[]" value="" />';
  schooluniversity_nameCell.innerHTML = '<input class="table-input" type="text" name="schooluniversity_name[]" value="" />';
  percentagegradeCell.innerHTML = '<input class="table-input" type="text" name="percentagegrade[]" value="" />';
  graduation_yearCell.innerHTML = '<input class="table-input" type="text" name="graduation_year[]" value="" />';
  actionCell.innerHTML = '<button class="table-input" type="button" onclick="deleteRow(this)">Delete</button>';
}
...
function add_projects_row() {
  ...
  project_titleCell.innerHTML = '<input class="table-input" type="text" name="project_title[]" value="" />';
  descriptionCell.innerHTML = '<input class="table-input" type="text" name="description[]" value="" />';
  roleCell.innerHTML = '<input class="table-input" type="text" name="role[]" value="" />';
  technologies_usedCell.innerHTML = '<input class="table-input" type="text" name="technologies_used[]" value="" />';
  actionCell.innerHTML = '<button class="table-input" type="button" onclick="deleteRow(this)">Delete</button>';
}
...
function add_publications_row() {
  ...
  titleCell.innerHTML = '<input class="table-input" type="text" name="title[]" value="" />';
  publisherCell.innerHTML = '<input class="table-input" type="text" name="publisher[]" value="" />';
  publication_dateCell.innerHTML = '<input class="table-input" type="date" name="publication_date[]" value="" />';
  actionCell.innerHTML = '<button class="table-input" type="button" onclick="deleteRow(this)">Delete</button>';
}
...
function add_references_row() {
  ...
  name1Cell.innerHTML = '<input class="table-input" type="text" name="name1[]" value="" />';
  titlepositionCell.innerHTML = '<input class="table-input" type="text" name="titleposition[]" value="" />';
  companyorganizationCell.innerHTML = '<input class="table-input" type="text" name="rcompanyorganization[]" value="" />';
  contact_information_phoneemailCell.innerHTML = '<input class="table-input" type="text" name="contact_information_phoneemail[]" value="" />';
  actionCell.innerHTML = '<button class="table-input" type="button" onclick="deleteRow(this)">Delete</button>';
}
...
function add_technical_skills_row() {
  ...
  skillsCell.innerHTML = '<input class="table-input" type="text" name="technical-skills[]" value="" />';
...
function add_soft_skills_row() {
  ...
  var actionCell = newRow.insertCell(2);
  levelCell.className = "star-wrapper"
  skillsCell.innerHTML = '<input class="table-input" type="text" name="soft-skills[]" value="" />';
  levelCell.innerHTML = '<a class="fas fa-star s1"></a> <a class="fas fa-star s2"></a> <a class="fas fa-star s3"></a> <a class="fas fa-star s4"></a> <a class="fas fa-star s5"></a> <input type="hidden" class="rating-input" value="0">';
  actionCell.innerHTML = '<button class="table-input" type="button" onclick="deleteRow(this)">Delete</button>';
  var rowIndex = newRow.rowIndex;
  add_star_wrapper_row(tableName, rowIndex);
...
function add_language_proficiency_row() {
  ...
  languageCell.innerHTML = '<input class="table-input" type="text" name="language[]" value="" />';
  readCell.innerHTML = '<input class="table-input" type="checkbox" name="read[]" value="" />';
  writeCell.innerHTML = '<input class="table-input" type="checkbox" name="write[]" value="" />';
  speakCell.innerHTML = '<input class="table-input" type="checkbox" name="speak[]" value="" />';
  actionCell.innerHTML = '<button class="table-input" type="button" onclick="deleteRow(this)">Delete</button>';
}
...
function get_career_history_data() {
  var table = document.getElementById("career-history").getElementsByTagName('tbody')[0];
  var data = [];
  for (var i = 0; i < table.rows.length; i++) {
    ...
    var curr_date = row.cells[6].querySelector('input').checked;
